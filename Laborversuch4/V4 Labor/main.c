	...
	switch (inputBuffer[0])		// erster Buchstabe des Befehls
	{
		...
		case 'd':
			if(inputBuffer[1] == 'c')
			{
				...
				sscanf(inputBuffer,"dc%d",&zahl);
//				strcpy(tmp, &inputBuffer[2]);
				lcd_send_cmd (zahl);
...
//					}
//				}
					sscanf(inputBuffer,"dd%d",&zahl);
				lcd_send_data((char)zahl);
			}
		...
		case 't':
			...
			sprintf((char*)output, "%u.%2u C", temp_c/100, temp_c%100);
